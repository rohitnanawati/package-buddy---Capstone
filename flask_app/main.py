...
cloud_vision_key=os.path.join(BASE_PATH,'google_cloudvision_key.json')
os.environ["GOOGLE_APPLICATION_CREDENTIALS"]= cloud_vision_key
...
MODEL_PATH = os.path.join(BASE_PATH,'static/models/PackageBuddyy_ner')
...
def index():
    if request.method == "POST":
        ...
        if ext.lower() in ['png','jpg','jpeg']:
            ...
            print(results)
            return render_template('upload.html',fileupload=True, tables=[results.T.to_html()], titles=[''],image_filename=filename,height=hei)
...
def pipeline_model(imagepath, model):
  ...
  def Predict(data,model):
    ...
    for ent in doc.ents:
        ...
        #print(text)
        dataF=pd.DataFrame(data=[text],columns=label,index=['Prediction'])  
    return(dataF)
